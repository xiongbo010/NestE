...
class NestE_H(Model):
	def __init__(self, ent_tot, rel_tot, meta_rel_tot, dim=50, seed=123):
		super(NestE_H, self).__init__(ent_tot, rel_tot)
		...
		self.meta_rel_s = nn.Embedding(self.meta_rel_tot, 9*self.dim)
		self.meta_rel_x = nn.Embedding(self.meta_rel_tot, 9*self.dim)
		self.meta_rel_y = nn.Embedding(self.meta_rel_tot, 9*self.dim)
		self.meta_rel_z = nn.Embedding(self.meta_rel_tot, 9*self.dim)
	...
	def init_weights(self):
		...
		s, x, y, z = self.quaternion_init(self.meta_rel_tot, 9*self.dim)
		s, x, y, z = torch.from_numpy(s), torch.from_numpy(x), torch.from_numpy(y), torch.from_numpy(z)
	...
	def forward(self, data):
		...
		score = self.score(s_h, x_h, y_h, z_h, s_t, x_t, y_t, z_t, s_r, x_r, y_r, z_r)
		return score
	...
	def forward_meta(self, data):
		...
		# print(self.meta_rel_s.weight.data.shape, batch_R.shape)

		s_R = self.meta_rel_s(batch_R).reshape(-1,3,3,self.dim)
		x_R = self.meta_rel_x(batch_R).reshape(-1,3,3,self.dim)
		y_R = self.meta_rel_y(batch_R).reshape(-1,3,3,self.dim)
		z_R = self.meta_rel_z(batch_R).reshape(-1,3,3,self.dim)

		s_H = torch.stack((s_h1, s_r1, s_t1),1).unsqueeze(2)
		x_H = torch.stack((x_h1, x_r1, x_t1),1).unsqueeze(2)
		y_H = torch.stack((y_h1, y_r1, y_t1),1).unsqueeze(2)
		z_H = torch.stack((z_h1, z_r1, z_t1),1).unsqueeze(2)
		...
		score_meta = self.meta_score(s_H, x_H, y_H, z_H, s_R, x_R, y_R, z_R, s_T, x_T, y_T, z_T)
		return score_meta
