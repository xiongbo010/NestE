...
from openke.module.model import BiVE_D
...
try:
	...
	BiVE_D.load_checkpoint('./checkpoint/{}/BiVE_D/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.ckpt'.format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial))
except:
	...
	trainer.run()
	os.makedirs('./checkpoint/{}/BiVE_D'.format(args.data), exist_ok=True)
	BiVE_D.save_checkpoint('./checkpoint/{}/BiVE_D/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.ckpt'.format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial))

if args.lp:
	test_dataloader = TestDataLoader("./benchmarks/{}/base/".format(args.data), "link")
	tester = Tester(model=BiVE_D, data_loader=test_dataloader, use_gpu=True)
...
if args.lp:
	os.makedirs("./result_test/{}/BiVE_D/LP".format(args.data), exist_ok=True)
	with open("./result_test/{}/BiVE_D/LP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("LP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))

elif args.tp:
	os.makedirs("./result_test/{}/BiVE_D/TP".format(args.data), exist_ok=True)
	with open("./result_test/{}/BiVE_D/TP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("TP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))

elif args.clp:
	os.makedirs("./result_test/{}/BiVE_D/CLP".format(args.data), exist_ok=True)
	with open("./result_test/{}/BiVE_D/CLP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("CLP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))
