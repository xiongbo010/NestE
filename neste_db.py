...
from openke.module.model import NestE_DB
...
try:
	...
	NestE_DB.load_checkpoint('./checkpoint/{}/NestE_DB/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.ckpt'.format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial))
except:
	...
	trainer.run()
	os.makedirs('./checkpoint/{}/NestE_DB'.format(args.data), exist_ok=True)
	NestE_DB.save_checkpoint('./checkpoint/{}/NestE_DB/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.ckpt'.format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial))

if args.lp:
	test_dataloader = TestDataLoader("./benchmarks/{}/base/".format(args.data), "link")
	tester = Tester(model=NestE_DB, data_loader=test_dataloader, use_gpu=True)
...
if args.lp:
	os.makedirs("./result_test/{}/NestE_DB/LP".format(args.data), exist_ok=True)
	with open("./result_test/{}/NestE_DB/LP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("LP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))

elif args.tp:
	os.makedirs("./result_test/{}/NestE_DB/TP".format(args.data), exist_ok=True)
	with open("./result_test/{}/NestE_DB/TP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("TP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))

elif args.clp:
	os.makedirs("./result_test/{}/NestE_DB/CLP".format(args.data), exist_ok=True)
	with open("./result_test/{}/NestE_DB/CLP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("CLP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))