...
from openke.module.model import NestE_HB
...
try:
	...
	NestE_HB.load_checkpoint('./checkpoint/{}/NestE_HB/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.ckpt'.format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial))
except:
	...
	trainer.run()
	os.makedirs('./checkpoint/{}/NestE_HB'.format(args.data), exist_ok=True)
	NestE_HB.save_checkpoint('./checkpoint/{}/NestE_HB/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.ckpt'.format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial))

if args.lp:
	test_dataloader = TestDataLoader("./benchmarks/{}/base/".format(args.data), "link")
	tester = Tester(model=NestE_HB, data_loader=test_dataloader, use_gpu=True)
	mr, mrr, hit10, hit3, hit1 = tester.run_link_prediction(type_constrain=False)

elif args.tp:
	test_dataloader = TestDataLoader("./benchmarks/{}/meta_7/".format(args.data), "link")
	tester = Tester(model=NestE_HB, data_loader=test_dataloader, use_gpu=True)
...
if args.lp:
	os.makedirs("./result_test/{}/NestE_HB/LP".format(args.data), exist_ok=True)
	with open("./result_test/{}/NestE_HB/LP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("LP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))

elif args.tp:
	os.makedirs("./result_test/{}/NestE_HB/TP".format(args.data), exist_ok=True)
	with open("./result_test/{}/NestE_HB/TP/alpha_{}_regul_{}_meta_7_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("TP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))

elif args.clp:
	os.makedirs("./result_test/{}/NestE_HB/CLP".format(args.data), exist_ok=True)
	with open("./result_test/{}/NestE_HB/CLP/alpha_{}_regul_{}_meta_{}_aug_{}_epoch_{}-{}.txt".format(args.data, args.alpha, args.regul, args.meta, args.aug, args.epoch, args.serial), 'w') as f:
		f.write("CLP: {} {} {} {} {}\n".format(mr, mrr, hit10, hit3, hit1))