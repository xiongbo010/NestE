...
class NestE_DB(Model):
	def __init__(self, ent_tot, rel_tot, meta_rel_tot, dim=50, seed=123):
		super(NestE_DB, self).__init__(ent_tot, rel_tot)
		...
		self.meta_rel_s = nn.Embedding(self.meta_rel_tot, 16*self.dim)
		self.meta_rel_x = nn.Embedding(self.meta_rel_tot, 16*self.dim)
		self.meta_rel_y = nn.Embedding(self.meta_rel_tot, 16*self.dim)
		self.meta_rel_z = nn.Embedding(self.meta_rel_tot, 16*self.dim)

		self.meta_rel_p_s = nn.Embedding(self.meta_rel_tot, 4*self.dim)
		self.meta_rel_p_x = nn.Embedding(self.meta_rel_tot, 4*self.dim)
		self.meta_rel_p_y = nn.Embedding(self.meta_rel_tot, 4*self.dim)
		self.meta_rel_p_z = nn.Embedding(self.meta_rel_tot, 4*self.dim)
	...
	def init_weights(self):
		...
		s, x, y, z = self.quaternion_init(self.meta_rel_tot, 16*self.dim)
		s, x, y, z = torch.from_numpy(s), torch.from_numpy(x), torch.from_numpy(y), torch.from_numpy(z)
		...
		s, x, y, z = self.quaternion_init(self.meta_rel_tot, 4*self.dim)
		s, x, y, z = torch.from_numpy(s), torch.from_numpy(x), torch.from_numpy(y), torch.from_numpy(z)
	...
	def score(self, s_h, x_h, y_h, z_h, s_t, x_t, y_t, z_t, s_r, x_r, y_r, z_r):
	...
	def block_prod(self, e, R):
		return torch.einsum('ijkz,iknz->ijnz', R, e).squeeze(-2)


	def meta_score(self, s_h, x_h, y_h, z_h, s_r, x_r, y_r, z_r, s_t, x_t, y_t, z_t):
		denominator_r = torch.sqrt(s_r ** 2 + x_r ** 2 + y_r ** 2 + z_r ** 2)
		s_r = s_r / denominator_r
		x_r = x_r / denominator_r
		y_r = y_r / denominator_r
		...
		A = self.block_prod(s_h, s_r) - self.block_prod(x_h, x_r) + self.block_prod(y_h, y_r) + self.block_prod(z_h, z_r)
		B = self.block_prod(s_h, x_r) + self.block_prod(x_h, s_r) - self.block_prod(y_h, z_r) + self.block_prod(z_h, y_r)
		C = self.block_prod(s_h, y_r) + self.block_prod(y_h, s_r) + self.block_prod(z_h, x_r) - self.block_prod(x_h, z_r)
		D = self.block_prod(s_h, z_r) + self.block_prod(z_h, s_r) + self.block_prod(x_h, y_r) - self.block_prod(y_h, x_r)

		score_R = (A * s_t + B * x_t + C * y_t + D * z_t)
		score_R = score_R.reshape(score_R.shape[0],-1)
		return torch.sum(score_R, -1)
	...
	def forward(self, data):
		...
		score = self.score(s_h + s_r_p, x_h + x_r_p, y_h + y_r_p, z_h + z_r_p, s_t, x_t, y_t, z_t, s_r, x_r, y_r, z_r)
		return score
	...
	def forward_meta(self, data):
		...
		s_R = self.meta_rel_s(batch_R).reshape(-1,4,4,self.dim)
		x_R = self.meta_rel_x(batch_R).reshape(-1,4,4,self.dim)
		y_R = self.meta_rel_y(batch_R).reshape(-1,4,4,self.dim)
		z_R = self.meta_rel_z(batch_R).reshape(-1,4,4,self.dim)

		s_R_p = self.meta_rel_p_s(batch_R).reshape(-1,4,1,self.dim)
		x_R_p = self.meta_rel_p_x(batch_R).reshape(-1,4,1,self.dim)
		y_R_p = self.meta_rel_p_y(batch_R).reshape(-1,4,1,self.dim)
		z_R_p = self.meta_rel_p_z(batch_R).reshape(-1,4,1,self.dim)

		s_H = torch.stack((s_h1, s_r1, s_r1_p, s_t1),1).unsqueeze(2)
		x_H = torch.stack((x_h1, x_r1, x_r1_p, x_t1),1).unsqueeze(2)
		y_H = torch.stack((y_h1, y_r1, y_r1_p, y_t1),1).unsqueeze(2)
		z_H = torch.stack((z_h1, z_r1, z_r1_p, z_t1),1).unsqueeze(2)

		s_T = torch.stack((s_h2, s_r2, s_r2_p, s_t2),1)
		x_T = torch.stack((x_h2, x_r2, x_r2_p, x_t2),1)
		y_T = torch.stack((y_h2, y_r2, y_r2_p, y_t2),1)
		z_T = torch.stack((z_h2, z_r2, z_r2_p, z_t2),1)
		# print(s_H.shape, s_R_p.shape, s_R.shape,s_T.shape)
		score_meta = self.meta_score(s_H + s_R_p, x_H + x_R_p, y_H + y_R_p, z_H + z_R_p, s_R, x_R, y_R, z_R, s_T, x_T, y_T, z_T)
		return score_meta
	...
	def regularization_meta(self, data):
		...
		s_H = torch.stack((s_h1, s_r1, s_t1),1)
		x_H = torch.stack((x_h1, x_r1, x_t1),1)
		y_H = torch.stack((y_h1, y_r1, y_t1),1)
		z_H = torch.stack((z_h1, z_r1, z_t1),1)

		s_T = torch.stack((s_h2, s_r2, s_t2),1)
		x_T = torch.stack((x_h2, x_r2, x_t2),1)
		y_T = torch.stack((y_h2, y_r2, y_t2),1)
		z_T = torch.stack((z_h2, z_r2, z_t2),1)
